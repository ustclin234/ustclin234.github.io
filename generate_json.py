...
def generate_pdf_json(file_folder, item='qsshouban2',save_folder='./json_files/'):
    files = sorted([s for s in os.listdir(file_folder) if s.endswith('.pdf')])
    contents = {
        "pdfs": files
    }
    js = json.dumps(contents, sort_keys=True,indent=4,separators=(',',':'),ensure_ascii=False)
...
generate_pdf_json('./qsshouban2/', 'qsshouban2')